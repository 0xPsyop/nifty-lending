...
contract NiftyLoan {
   ...
   event NewLoanCreated(address indexed _nftAddress, address indexed _lender, uint256 indexed _tokenId, uint256 _requiredAmount, uint256 _interestPercentage, uint256 _loanTerm);
   ...
   event LoanFunded(Loan loan);
    ...
    modifier isListed(address _nftAddress, uint256 _tokenId) {
      ...
      if(loan.requiredAmount == 0 ) revert LoanNotListed(_nftAddress,_tokenId);
      _;
    }
    ...
    modifier isNotListed(address _nftAddress, uint256 _tokenId){
     Loan memory loan = loans[_nftAddress][_tokenId];
     if(loan.requiredAmount> 0) revert LoanAlreadyListed(_nftAddress, _tokenId);
      _;
    }

    //@dev check if the caller is the owner of the specific token
     modifier isOwner(address _nftAddress, uint256 _tokenId){
      if(IERC721(_nftAddress).ownerOf(_tokenId) != msg.sender) revert NotOwner();
      _;
    }
   ...
   */
    ...
    function createLoan(
      ...
      uint256 _loanTerm
       ...
       require(_requiredAmount > 0 && _loanTerm > 0 && _interestPercentage> 0 , "Not a valid loan");
       
       //get the token approval here or do it before calling dis func
       Loan memory newLoan = Loan(_nftAddress, payable(msg.sender), _tokenId, _requiredAmount, _interestPercentage, _loanTerm , false);
       loans[_nftAddress][_tokenId] = newLoan;

      emit NewLoanCreated(_nftAddress, msg.sender, _tokenId, _requiredAmount, _interestPercentage, _loanTerm );
    }
  ...
  // @dev update the already listed loan details (literally changing the values in the Loan struct)
    function updateLoan(
      ...
      emit LoanUpdated(_nftAddress, _tokenId, _newRequiredAmount, _newInterestPercentage, _newLoanTerm);
    }
